...
<template>
  <main class="os" ref="root">
    ...
    <div class="dust" aria-hidden="true">
      <span v-for="n in 26" :key="n" class="p" :style="`--i:${n}`"></span>
    </div>

    <section class="desktop" v-if="!err">
      <FolderIcon v-for="p in projects" :key="p.id" :project="p" @open="wm.openProject" />
    </section>

    <section v-else class="error">
      <div class="card">
        <div class="h">Couldn’t reach the API</div>
        <div class="s">{{ err }}</div>
        <div class="s">Start the server in <code>server/</code> on port <code>3001</code>, then refresh.</div>
      </div>
    </section>

    <ProjectWindow
      v-for="w in wm.windows.value"
      :key="w.id"
      :model="w"
      :style="{ zIndex: w.z }"
      @close="wm.close(w.id)"
      @minimize="wm.minimize(w.id)"
      @maximize="wm.maximize(w.id)"
      @focus="wm.focus(w.id)"
    />
  </main>
</template>
...
.wallpaper {
  position: absolute;
  inset: -3%;
  background: radial-gradient(1200px 700px at 30% 20%, rgba(120, 200, 255, 0.18), transparent 60%),
    radial-gradient(900px 600px at 70% 60%, rgba(190, 120, 255, 0.14), transparent 55%),
    linear-gradient(180deg, #070814, #05060a 55%, #04050a);
